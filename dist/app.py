...
import pytz
import csv # Added for export_all_dropoffs_csv
...
def create_app():
    ...
    def login():
        ...
        return render_template('login.html')
# alongside your other routes (like /dashboard, /add-tracking, etc.):
    ...
    # Admin Users Dashboard with Search
    ...
    def admin_users():
        ...
        # --- Search Drop-offs Logic ---
        search_user_query = request.args.get('search_user', '').strip()
        dropoffs_query = Tracking.query.order_by(Tracking.timestamp.desc()) # Note: changed from Dropoff to Tracking

        if search_user_query:
            matching_users = User.query.filter(User.phone_number.ilike(f'%{search_user_query}%')).all()
            matching_user_ids = [user.id for user in matching_users]

            if matching_user_ids:
                dropoffs_query = dropoffs_query.filter(Tracking.user_id.in_(matching_user_ids))
            else:
                dropoffs = []
                return render_template('admin/users.html', users=users, dropoffs=dropoffs, search_query=search_user_query)
    ...
    def export_dropoffs_csv():
        ...
        si = io.StringIO() # Use io.StringIO consistently
        cw = csv.writer(si)
    ...
    # CLI Command to initialize database and admin user
    ...
    def init_db_command():
        ...
        with app.app_context():
            ...
            admin_password = os.getenv('INITIAL_ADMIN_PASSWORD', 'password123')

            if not User.query.filter_by(is_admin=True).first(): # Check if any admin exists
                if not User.query.filter_by(phone_number=admin_phone).first(): # Check if specific admin user exists
                    print(f"Creating initial admin user: {admin_phone}")
                    admin_user = User(phone_number=admin_phone, is_admin=True)
                    admin_user.set_password(admin_password)
                    db.session.add(admin_user)
                    db.session.commit()
                    print("Initial admin user created. Please change password after first login.")
                else:
                    print(f"User {admin_phone} already exists. Ensure it's an admin if intended.")
            else:
                print("An admin user already exists. No new admin user created.")
